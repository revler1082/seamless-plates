...
set tile_dir=\\ssaweb\projects\SeamlessMSPlates\images\w\e_m_and_s\normal\
set zoom_level=10
...
for /F %%i in (%~dp0rows.txt) do (
	...
	for /F %%j in (%~dp0columns.txt) do (
		...
		IF NOT EXIST "%scale_dir%%%i-%%j.png" (
			...
			convert "%scale_dir%%%i-%%j.png" -scale 25.0000%% -crop 256x192 -set filename:tile "%zoom_level%\\%%[fx:page.x/256+!xoff!]\\%%[fx:page.y/192+!yoff!]" +repage +adjoin "%tile_dir%%%[filename:tile].png"
		)
