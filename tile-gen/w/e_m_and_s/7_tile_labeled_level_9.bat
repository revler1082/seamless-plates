...
set tile_dir=\\ssaweb\projects\SeamlessMSPlates\images\w\e_m_and_s\labeled\
set zoom_level=9
...
for /F %%i in (%~dp0rows.txt) do (
	...
	for /F %%j in (%~dp0columns.txt) do (
		...
		IF NOT EXIST "%label_dir%%%i-%%j.png" (
			...
			convert "%label_dir%%%i-%%j.png" -scale 12.50000%% -crop 256x192 -set filename:tile "%zoom_level%\\%%[fx:page.x/256+!xoff!]\\%%[fx:page.y/192+!yoff!]" +repage +adjoin "%tile_dir%%%[filename:tile].png"
		)
