...
set tile_dir=\\ssaweb\projects\SeamlessMSPlates\images\s\e_m_and_s\labeled\
set zoom_level=9
...
for /L %%a in (0,1,512) do ( IF NOT EXIST "%tile_dir%%zoom_level%\%%a" MKDIR %tile_dir%%zoom_level%\%%a )
...
for /F %%i in (%~dp0rows.txt) do (
	...
	for /F %%j in (%~dp0columns.txt) do (
		...
		IF NOT EXIST "%label_dir%%%j%%i.png" (
			echo %label_dir%%%j%%i does not exist, continuing
		) ELSE (
			echo "%label_dir%%%j%%i" exists
			...
			IF NOT EXIST "%tile_dir%%zoom_level%\!xoff!\!yoff!.png" (
				...
				convert "%label_dir%%%j%%i.png" -scale 12.50000%% -crop 257x256 -set filename:tile "%zoom_level%\\%%[fx:page.x/257+!xoff!]\\%%[fx:page.y/256+!yoff!]" +repage +adjoin "%tile_dir%%%[filename:tile].png"
			) ELSE (
				for %%m in ("%label_dir%%%j%%i.png") do set label_time=%%~tm
				...
				IF !label_time! GTR !tile_time! (
					echo     %label_dir%%%j%%i is newer than existing tile imagery
					echo     creating level %zoom_level% zoom tiles
					convert "%label_dir%%%j%%i.png" -scale 12.50000%% -crop 257x256 -set filename:tile "%zoom_level%\\%%[fx:page.x/257+!xoff!]\\%%[fx:page.y/256+!yoff!]" +repage +adjoin "%tile_dir%%%[filename:tile].png"
				) ELSE (
