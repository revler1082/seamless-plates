...
set tile_dir=\\ssaweb\projects\SeamlessMSPlates\images\s\e_m_and_s\normal\
set zoom_level=10
...
for /L %%a in (0,1,1024) do ( IF NOT EXIST "%tile_dir%%zoom_level%\%%a" MKDIR %tile_dir%%zoom_level%\%%a )
	...
	for /F %%j in (%~dp0columns.txt) do (
		...
		IF NOT EXIST "%scale_dir%%%j%%i.png" (
			...
			IF NOT EXIST "%tile_dir%%zoom_level%\!xoff!\!yoff!.png" (
				...
				convert "%scale%%%j%%i.png" -scale 25.0000%% -crop 257x256 -set filename:tile "%zoom_level%\\%%[fx:page.x/257+!xoff!]\\%%[fx:page.y/256+!yoff!]" +repage +adjoin "%tile_dir%%%[filename:tile].png"
			) ELSE (
				...
				IF !scale_time! GTR !tile_time! (
					...
					echo     creating level %zoom_level% zoom tiles
					convert "%scale_dir%%%j%%i.png" -scale 25.0000%% -crop 257x256 -set filename:tile "%zoom_level%\\%%[fx:page.x/257+!xoff!]\\%%[fx:page.y/256+!yoff!]" +repage +adjoin "%tile_dir%%%[filename:tile].png"
				) ELSE (
