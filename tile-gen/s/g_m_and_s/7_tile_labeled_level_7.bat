	...
	for /F %%j in (%~dp0columns.txt) do (
		...
		IF NOT EXIST "%label_dir%%%j%%i.png" (
			...
			IF NOT EXIST "%tile_dir%%zoom_level%\!xoff!\!yoff!.png" (
				...
				convert "%label_dir%%%j%%i.png" -scale 3.12500%% -crop 257x256 -set filename:tile "%zoom_level%\\%%[fx:page.x/257+!xoff!]\\%%[fx:page.y/256+!yoff!]" +repage +adjoin "%tile_dir%%%[filename:tile].png"
			) ELSE (
