...
for /f %%f in ('dir /b "%preprocess_dir%*.png"') do (
	IF NOT EXIST "%crop_dir%%%~nf.png" (
		...
		for /F "tokens=1-5 delims= " %%A in ('convert %preprocess_dir%%%f -format "0x%%[fx:int(h**0.044712)] 0x%%[fx:int(h**0.045528)] %%[fx:int(w**0.023743)]x0" info:') do convert %preprocess_dir%%%f -gravity South -chop %%A -gravity North -chop %%B -shave %%C +repage -trim +repage -shave 1x1 +repage "%crop_dir%%%~nf.png"
	) ELSE (
