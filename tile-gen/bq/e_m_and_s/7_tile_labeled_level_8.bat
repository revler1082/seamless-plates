	...
	for /F %%j in (%~dp0columns.txt) do (
		...
		IF NOT EXIST "%label_dir%%%i-%%j.png" (
			...
			IF NOT EXIST "%tile_dir%%zoom_level%\!xoff!\!yoff!.png" (
				...
				convert "%label_dir%%%i-%%j.png" -scale 6.25000%% -crop 256x190 -set filename:tile "%zoom_level%\\%%[fx:page.x/256+!xoff!]\\%%[fx:page.y/190+!yoff!]" +repage +adjoin "%tile_dir%%%[filename:tile].png"
			) ELSE (
