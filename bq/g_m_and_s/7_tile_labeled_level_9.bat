...
set tile_dir=\\ssaweb\projects\SeamlessMSPlates\images\bq\g_m_and_s\labeled\
set zoom_level=9
	...
	for /F %%j in (%~dp0columns.txt) do (
		...
		IF NOT EXIST "%label_dir%%%i-%%j.png" (
			...
			IF NOT EXIST "%tile_dir%%zoom_level%\!xoff!\!yoff!.png" (
				...
				convert "%label_dir%%%i-%%j.png" -scale 12.50000%% -crop 256x190 -set filename:tile "%zoom_level%\\%%[fx:page.x/256+!xoff!]\\%%[fx:page.y/190+!yoff!]" +repage +adjoin "%tile_dir%%%[filename:tile].png"
			) ELSE (
