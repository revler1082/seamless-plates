...
set tile_dir=\\ssaweb\projects\SeamlessMSPlates\images\bq\e_m_and_s\normal\
set zoom_level=10
	...
	for /F %%j in (%~dp0columns.txt) do (
		...
		IF NOT EXIST "%scale_dir%%%i-%%j.png" (
			...
			IF NOT EXIST "%tile_dir%%zoom_level%\!xoff!\!yoff!.png" (
				...
				convert "%scale_dir%%%i-%%j.png" -scale 25.0000%% -crop 256x190 -set filename:tile "%zoom_level%\\%%[fx:page.x/256+!xoff!]\\%%[fx:page.y/190+!yoff!]" +repage +adjoin "%tile_dir%%%[filename:tile].png"
			) ELSE (
				for %%m in ("%scale_dir%%%i-%%j.png") do set scale_time=%%~tm
				...
				IF !scale_time! GTR !tile_time! (
					echo     %scale_dir%%%i-%%j is newer than existing tile imagery
					echo     creating level %zoom_level% zoom tiles
					convert "%scale_dir%%%i-%%j.png" -scale 25.0000%% -crop 256x190 -set filename:tile "%zoom_level%\\%%[fx:page.x/256+!xoff!]\\%%[fx:page.y/190+!yoff!]" +repage +adjoin "%tile_dir%%%[filename:tile].png"
				) ELSE (
