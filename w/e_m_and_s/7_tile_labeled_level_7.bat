...
for /F %%i in (%~dp0rows.txt) do (
	...
	for /F %%j in (%~dp0columns.txt) do (
		...
		IF NOT EXIST "%label_dir%%%i-%%j.png" (
			...
			convert "%label_dir%%%i-%%j.png" -scale 3.12500%% -crop 256x192 -set filename:tile "%zoom_level%\\%%[fx:page.x/256+!xoff!]\\%%[fx:page.y/192+!yoff!]" +repage +adjoin "%tile_dir%%%[filename:tile].png"
		)
